...
Object.entries(mapping).forEach(([oldName, newName]) => {
  ...
  // Find match in files array to be sure Case and characters match exactly
  const actualFile = files.find(f => f.trim() === oldName.trim());
  
  if (actualFile) {
    const actualOldPath = path.join(dir, actualFile);
    fs.renameSync(actualOldPath, newPath);
    console.log(`Renamed: ${actualFile} -> ${newName}`);
  } else {
